...
1. **UI Core** ([`src/components/ui`](./components)): Nguyên tử giao diện chuẩn hóa (Buttons, Inputs, Cards, Tables, Dialogs...).
...
## II. Chi Tiết Các UI Components Core (`design-system-kit/components`)

---

### 1. Nhóm Nút Bấm & Điều Hướng (Button Family)

Gồm 3 biến thể component xuất phát từ file [`Button.tsx`](./components/Button.tsx):
...
- Hỗ trợ hàm tiện ích [`getButtonClasses`](./components/Button.tsx#L74) và [`normalizeButtonSize`](./components/Button.tsx#L51).
...
### 2. FormField ([`FormField.tsx`](./components/FormField.tsx))
...
### 3. Input ([`Input.tsx`](./components/Input.tsx))
...
### 4. Textarea ([`Textarea.tsx`](./components/Textarea.tsx))
...
### 5. Checkbox ([`Checkbox.tsx`](./components/Checkbox.tsx)) & Switch ([`Switch.tsx`](./components/Switch.tsx))
...
### 6. Select ([`Select.tsx`](./components/Select.tsx))
...
#### 🎯 Vai trò & Đặc điểm
- **`DatePickerInput`** ([`DatePickerInput.tsx`](./components/DatePickerInput.tsx)):
...
- **`DateTimePickerInput`** ([`DateTimePickerInput.tsx`](./components/DateTimePickerInput.tsx)):
...
1. **`SearchInput`** ([`SearchInput.tsx`](./components/SearchInput.tsx)):
...
2. **`FilterDropdown`** ([`FilterDropdown.tsx`](./components/FilterDropdown.tsx)):
...
3. **`SearchToolbar`** ([`SearchToolbar.tsx`](./components/SearchToolbar.tsx)):
...
#### 🎯 Vai trò & Phân biệt
- **`Card Family`** ([`Card.tsx`](./components/Card.tsx)):
...
- **`StatCard`** ([`StatCard.tsx`](./components/StatCard.tsx)):
...
| :--- | :--- | :--- |
| **`Table` Suite** | [`Table.tsx`](./components/Table.tsx) | Bộ nguyên tử HTML table (`TableContainer`, `TableHeader`, `TableBody`, `TableRow`, `TableHead`, `TableCell`). Dùng khi cần vẽ bảng cấu trúc tùy biến phức tạp (merge cell, custom row grouping). |
| **`ResponsiveTable<T>`** | [`ResponsiveTable.tsx`](./components/ResponsiveTable.tsx) | **Khuyên dùng cho mọi trang danh sách**. Tự động hiển thị bảng cuộn trên Desktop (`md+`) và **chuyển thành Card Stack trên Mobile** (`<md`). Tích hợp sẵn Skeleton, Error Retry, Empty state, Pagination và `onRowClick`. |
| **`Pagination`** | [`Pagination.tsx`](./components/Pagination.tsx) | Thanh chuyển trang thông minh, tự động rút gọn dấu ba chấm (`...`) và hiển thị cô đọng trên mobile. |
...
#### 🎯 Vai trò & Phân biệt
- **`Badge`** ([`Badge.tsx`](./components/Badge.tsx)):
...
- **`StatusBadge`** ([`StatusBadge.tsx`](./components/StatusBadge.tsx)):
...
#### 🎯 Vai trò & Đặc điểm
- **`Modal`** ([`Modal.tsx`](./components/Modal.tsx)):
...
- **`ConfirmDialog`** ([`ConfirmDialog.tsx`](./components/ConfirmDialog.tsx)):
...
Gồm 5 component chuẩn hóa bố cục trang trong file [`PageUI.tsx`](./components/PageUI.tsx):
...
### 14. Chuyển Đổi Ngôn Ngữ ([`LanguageSwitcher.tsx`](./components/LanguageSwitcher.tsx))
...
### 15. Nhóm Điều Hướng Tab (Tabs Suite - [`Tabs.tsx`](./components/Tabs.tsx))
...
#### 💡 Ví dụ sử dụng:
```tsx
import { Tabs, TabsList, TabsTrigger, TabsContent, TabsBar } from '@/design-system'
...
### 16. Hộp Thông Báo & Cảnh Báo (Alert Suite - [`Alert.tsx`](./components/Alert.tsx))
...
#### 💡 Ví dụ sử dụng:
```tsx
import { Alert, AlertTitle, AlertDescription, Button } from '@/design-system'
...
### 17. Thanh Tiến Độ (ProgressBar - [`ProgressBar.tsx`](./components/ProgressBar.tsx))
...
### 18. Ảnh Đại Diện Hội Viên & Nhân Viên (Avatar Suite - [`Avatar.tsx`](./components/Avatar.tsx))
...
### 19. Menu Co Giãn (Accordion Suite - [`Accordion.tsx`](./components/Accordion.tsx))
...
#### 💡 Ví dụ sử dụng:
```tsx
import { Accordion, AccordionItem, AccordionTrigger, AccordionContent, AccordionGroup } from '@/design-system'
...
### 20. Trình Hướng Dẫn Từng Bước (Stepper - [`Stepper.tsx`](./components/Stepper.tsx))
...
### 21. Khung Giữ Chỗ Tải Dữ Liệu Nguyên Tử (Skeleton Suite - [`Skeleton.tsx`](./components/Skeleton.tsx))
...
#### 💡 Ví dụ sử dụng:
```tsx
import { Skeleton, SkeletonText, SkeletonCircle } from '@/design-system'
...
- Tích hợp 4 cụm tính năng thiết yếu:
  ...
  2. **Chuyển đổi ngôn ngữ**: Tích hợp [`LanguageSwitcher`](./components/LanguageSwitcher.tsx).
...
- Xử lý các tình huống lỗi mạng/mất kết nối server với thông báo [`PageErrorState`](./components/PageUI.tsx) kèm nút "Thử lại" hoặc "Đăng xuất".
...
#### 🎯 Vai trò & Đặc điểm
- Là wrapper tái sử dụng component [`Pagination`](./components/Pagination.tsx) gốc, giúp chuẩn hóa giao diện phân trang trong toàn bộ các bảng quản lý của Owner (Danh sách nhân viên, Hóa đơn, Báo cáo tài chính).

---

## IV. Bảng Tra Cứu Toàn Bộ Mã Nguồn & Exports

### 1. Thư Mục UI Core (`design-system-kit/components`)

| Nhóm Chức Năng | Thành Phần Export | File Nguồn |
| :--- | :--- | :--- |
| **Buttons & Links** | `Button`, `ButtonLink`, `ButtonAnchor`, `ButtonContent`, `getButtonClasses`, `normalizeButtonSize` | [`Button.tsx`](./components/Button.tsx) |
| **Form Controls** | `Input`, `getInputClasses`, `normalizeInputSize` | [`Input.tsx`](./components/Input.tsx) |
| | `Textarea`, `getTextareaClasses` | [`Textarea.tsx`](./components/Textarea.tsx) |
| | `FormField` | [`FormField.tsx`](./components/FormField.tsx) |
| | `Checkbox`, `getCheckboxClasses` | [`Checkbox.tsx`](./components/Checkbox.tsx) |
| | `Switch`, `getSwitchClasses` | [`Switch.tsx`](./components/Switch.tsx) |
| | `Select` | [`Select.tsx`](./components/Select.tsx) |
| **Date & Time** | `DatePickerInput` | [`DatePickerInput.tsx`](./components/DatePickerInput.tsx) |
| | `DateTimePickerInput` | [`DateTimePickerInput.tsx`](./components/DateTimePickerInput.tsx) |
| **Search & Filtering** | `SearchInput` | [`SearchInput.tsx`](./components/SearchInput.tsx) |
| | `FilterDropdown` | [`FilterDropdown.tsx`](./components/FilterDropdown.tsx) |
| | `SearchToolbar` | [`SearchToolbar.tsx`](./components/SearchToolbar.tsx) |
| **Containers & Cards** | `Card`, `CardHeader`, `CardTitle`, `CardDescription`, `CardContent`, `CardFooter` | [`Card.tsx`](./components/Card.tsx) |
| | `StatCard` | [`StatCard.tsx`](./components/StatCard.tsx) |
| **Data Tables & Pagination** | `Table`, `TableContainer`, `TableHeader`, `TableBody`, `TableFooter`, `TableRow`, `TableHead`, `TableCell` | [`Table.tsx`](./components/Table.tsx) |
| | `ResponsiveTable` | [`ResponsiveTable.tsx`](./components/ResponsiveTable.tsx) |
| | `Pagination` | [`Pagination.tsx`](./components/Pagination.tsx) |
| **Badges & Feedback** | `Badge`, `getBadgeClasses`, `normalizeBadgeSize` | [`Badge.tsx`](./components/Badge.tsx) |
| | `StatusBadge` | [`StatusBadge.tsx`](./components/StatusBadge.tsx) |
| | `Modal` | [`Modal.tsx`](./components/Modal.tsx) |
| | `ConfirmDialog` | [`ConfirmDialog.tsx`](./components/ConfirmDialog.tsx) |
| **Page Layout & Utils** | `Page`, `PageHeader`, `PageSkeleton`, `PageEmptyState`, `PageErrorState` | [`PageUI.tsx`](./components/PageUI.tsx) |
| | `LanguageSwitcher` | [`LanguageSwitcher.tsx`](./components/LanguageSwitcher.tsx) |
| **Navigation & Tabs** | `Tabs`, `TabsList`, `TabsTrigger`, `TabsContent`, `TabsBar` | [`Tabs.tsx`](./components/Tabs.tsx) |
| **Alerts & Messages** | `Alert`, `AlertTitle`, `AlertDescription` | [`Alert.tsx`](./components/Alert.tsx) |
| **Progress & Metrics** | `ProgressBar` | [`ProgressBar.tsx`](./components/ProgressBar.tsx) |
| **Avatars & Users** | `Avatar`, `AvatarGroup` | [`Avatar.tsx`](./components/Avatar.tsx) |
| **Collapsible & FAQs** | `Accordion`, `AccordionItem`, `AccordionTrigger`, `AccordionContent`, `AccordionGroup` | [`Accordion.tsx`](./components/Accordion.tsx) |
| **Wizards & Steppers** | `Stepper` | [`Stepper.tsx`](./components/Stepper.tsx) |
| **Loading Skeletons** | `Skeleton`, `SkeletonText`, `SkeletonCircle` | [`Skeleton.tsx`](./components/Skeleton.tsx) |
